...
$web_upload_path = '/Operation/uploads/'; // Web-accessible path
        ...
        if ($action === 'save') {
            ...
            if (!empty($_POST['files_to_delete'])) {
                ...
                foreach ($files_to_delete as $file_path) {
                    ...
                    // Try to find the file in different formats
                    $found = false;
                ...
                for ($i = 0; $i < $total_files; $i++) {
                    if ($_FILES['documents']['error'][$i] === 0) {
                        ...
                        if (move_uploaded_file($file_tmp, $upload_path)) {
                            ...
                            $documents[] = $web_upload_path . $new_file_name;
                        }
                    }
                }
            }
                    ...
                    if (preg_match('/^data:image\/(\w+);base64,/', $photo_data, $type)) {
                        ...
                        if ($data !== false) {
                            ...
                            if (file_put_contents($file_path, $data)) {
                                $documents[] = $web_upload_path . $file_name;
                            }
                        }
                    }
                }
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        // Fix existing document paths if they're stored as relative paths
        $documents = json_decode($row['documents'] ?? '[]', true);
        if (!empty($documents)) {
            $fixed_documents = [];
            foreach ($documents as $doc) {
                // If it's a relative path, convert it to web path
                if (strpos($doc, 'uploads/') === 0) {
                    $filename = basename($doc);
                    $fixed_documents[] = $web_upload_path . $filename;
                } else {
                    $fixed_documents[] = $doc;
                }
            }
            $row['documents'] = json_encode($fixed_documents);
        }
        $records[] = $row;
    }
}
      ...
      <ul class="space-y-1">
        ...
        <li class="dropdown-item">
          ...
          <ul id="operation-menu" class="dropdown-menu bg-gray-700">
            ...
            <li>
              <a href="/Admin executive/Operation/operation_panel.php" class="submenu-link active">
    ...
    function viewRow(button) {
      ...
      if (currentDocuments && currentDocuments.length > 0) {
        ...
        currentDocuments.forEach((doc, index) => {
          ...
          // Handle image load error - try different paths
          img.onerror = function() {
            console.log('Image failed to load:', this.src);
            
            // Try to load with absolute path if it's relative
            if (this.src.startsWith('/Admin executive/Operation/uploads/')) {
              // Already absolute, try base URL
              const baseUrl = window.location.origin;
              const fullUrl = baseUrl + this.src;
              this.src = fullUrl;
              ...
              this.onerror = function() {
                ...
                errorDiv.innerHTML = `
                  ...
                  <div class="text-xs text-gray-500">${doc.split('/').pop()}</div>
                `;
    ...
    function editRow(button) {
      ...
      if (documents && documents.length > 0) {
        documents.forEach((doc, index) => {
          ...
          // Handle image error in edit mode
          img.onerror = function() {
            if (this.src.startsWith('/Admin executive/Operation/uploads/')) {
              const baseUrl = window.location.origin;
              const fullUrl = baseUrl + this.src;
